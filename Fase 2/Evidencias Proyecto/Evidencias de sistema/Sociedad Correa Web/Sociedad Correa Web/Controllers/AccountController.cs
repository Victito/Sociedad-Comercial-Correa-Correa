...
public class AccountController : Controller
{
    ...
    public async Task<IActionResult> Login(string nombreUsuario, string clave)
    {
        ...
        // Validar caracteres permitidos usando expresiones regulares
        var regexUsuario = new Regex(@"^[a-zA-Z0-9.-]+$"); // Solo letras, números, puntos, guiones y guiones bajos
        var regexClave = new Regex(@"^[a-zA-Z0-9]+$"); // Solo letras y números

        if (!regexUsuario.IsMatch(nombreUsuario) || !regexClave.IsMatch(clave))
        {
            ViewBag.ErrorMessage = "Los campos contienen caracteres no permitidos.";
            return View("~/Views/Home/Index.cshtml");
        }
